				...
				<form onSubmit={postContactClick}>
					<div className="flex flex-col md:flex-row">
						...
						<div className="w-full md:w-8/12 md:pl-16 md:px-3 h-full">
							<textarea name="message" id="message" cols="30" rows="4" placeholder={t('message')} className="resize-none h-40 placeholder-gray-500 border-1 border-lightBlue rounded px-3 py-1 w-full"></textarea>
						</div>
					</div>
					<div className="mt-6 flex flex-row justify-between">
						<button className={`rounded px-6 py-2 text-white bg-lightBlue2 shadow-lg hover:shadow-xl transition duration-200 ease-out ${loading ? 'bg-blue-300' : ''}`} disabled={loading} type='submit'> {t('sent')} </button>
						<div className="mr-3">
